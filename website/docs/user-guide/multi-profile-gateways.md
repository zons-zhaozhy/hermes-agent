...
<!-- merge: the local "### Rollback / hermes gateway migrate --standalone" section was dropped — upstream's multiplex-only topology removed that reverse command ("there is no `--standalone` reverse"), and the local section's transactional paragraphs are preserved verbatim in the section above. -->
