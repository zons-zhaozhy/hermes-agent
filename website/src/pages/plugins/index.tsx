...
function PluginCard({
  ...
  // merge: the local `pinUrl` const is gone — upstream's imported `pinUrl(plugin)` helper is used
  // at the card title instead; the local `installUrl` deep link is kept.
  ...
  return (
    ...
    >
      ...
      <div className={styles.cardInner}>
        ...
        {/* merge: kept the local `installUrl` (`hermes://plugin/install?repo=…#subdir&catalog_name&sha&enable=1`) for the Desktop link — it carries the reviewed pin and still routes through upstream's `plugin/install?repo=` branch; upstream's title attribute is added. */}
        {onPick ? (
          <button
        ...
        <header className={styles.hero}>
          ...
          <div className={styles.heroContent}>
            ...
            <p className={styles.heroSub}>
              {/* merge: kept the local hero subtitle — it names the page's own controls (Desktop review/install, copy the CLI command) and matches the "Install in Hermes" button; upstream's "Give Hermes new powers…" community line is dropped at this one spot. */}
