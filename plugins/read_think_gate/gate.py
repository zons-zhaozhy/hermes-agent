...
_FOUR_AXIS_KEYWORDS: dict[str, list[str]] = {
    "影响面": [
        "影响面",  # 2026-09-26: 轴名本身入表——拦截消息要求「影响面清单」但模型常写
        # 「影响面=…」等自然变体，裸轴名缺席致 judge 判 3/4 假拦截五轮（实测 0926）。
        # 轴名出现即证明模型在输出该轴，语义零误报。
        "[源码确认]", "[搜索推断]",
    ...
    "风险": [
        "风险",  # 2026-09-26: 同上，轴名入表（「根因」轴本就含轴名，此为补齐另三轴）
        "静默数据损坏", "向后不兼容", "缓存失效",
        "并发竞争", "异常路径被吞", "第三方依赖超时",
        "触发条件", "影响范围", "可恢复",
        "风险矩阵",
