...
# merge tradeoff: kept the local failure backoff (_BOT_POLL_FAILURE_BACKOFF_S + ``_bot_poll_retry_at``) over
# upstream's backoff-free variant; upstream's 5 s loop call-rate gate (below) is kept as well.
