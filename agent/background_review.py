...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • **The task outcome suggests a skill failed** — scan the final "
    "few turns. Did the conversation end with an error message, "
    "compilation failure, test failure, or the user saying 'this "
    "didn't work' / 'still broken'? If a loaded skill was involved "
    "and the task failed, identify which step went wrong and propose "
    "a concrete fix (wrong approach, missing prerequisite, incorrect "
    "command).\n"
    "  • **A loaded skill was not followed and the task still "
    "succeeded** — consider whether the skill's guidance is overly "
    "constraining or outdated. If the agent solved the task by "
    "ignoring a skill's prescribed steps, flag that skill for "
    "trimming.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • **A loaded skill was not followed and the task still "
    "succeeded** — consider whether the skill's guidance is overly "
    "constraining. If the agent solved the task by ignoring a "
    "skill's prescribed steps, flag that skill for trimming.\n\n"
