...
def _is_zai_thinking_conflict(exc: Exception, client: Any) -> bool:
    """ZAI thinking-capable models reject temperature with code 1210 and a message about
    thinking being unable to disable — the parameter name never appears, so the generic
    unsupported-parameter detector misses it. On the global endpoint this error family
    usually means the model is not provisioned for this key at all; the actionable fix is
    an explicit base_url pointing at the CN coding endpoint (fork)."""
    err_str = str(exc)
    base = str(getattr(client, "base_url", "")).lower()
    return ("1210" in err_str and "thinking" in err_str.lower()
            and ("z.ai" in base or "bigmodel" in base))


def _without_temperature_thinking(kwargs: dict) -> Optional[dict]:
    """Copy *kwargs* without ``temperature`` and the ``thinking`` extra_body key (they carry
    the same conflict); None when neither was on the wire, so an unchanged request is never
    re-sent."""
    retry_kwargs = dict(kwargs)
    changed = retry_kwargs.pop("temperature", None) is not None
    extra_body = retry_kwargs.get("extra_body")
    if isinstance(extra_body, dict) and "thinking" in extra_body:
        retry_kwargs["extra_body"] = {k: v for k, v in extra_body.items() if k != "thinking"}
        changed = True
    return retry_kwargs if changed else None
...
def _parameter_rungs(client: Any, max_tokens: Optional[int]) -> tuple:
    ...
    return (
        ...
        # ZAI thinking-capable models reject temperature with the same code 1210 and a
        # message about thinking being unable to disable — the parameter name never appears,
        # so strip temperature and the thinking extra_body (same conflict) once (fork).
        (lambda exc: _is_zai_thinking_conflict(exc, client), _without_temperature_thinking,
         "ZAI rejected the request with code 1210 (thinking-parameter conflict, message never "
         "names the parameter); retrying without temperature/thinking — if this persists, the "
         "model is likely not provisioned on this endpoint for this key; set an explicit "
         "base_url (CN coding endpoint) for this auxiliary task", None),
