...
async function fetchBoundConfigRecord(
  ...
  // merge: upstream extracted this into fetchBoundConfigRecord; the local
  // dial-priority customization is folded into the bound origin here so it
  // still rides every capability-scoped read.
  const origin = { ...capabilityScoped(profile ?? undefined), ...scopedDialPriority(profile) }
...
export function saveHermesConfig(
  ...
  // merge: upstream's resolveConfigWriteScope routes the write back to the
  // origin the matching read bound; the local dial-priority tag is kept on top.
