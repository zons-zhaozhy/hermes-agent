...
- **Context compression** (`agent/context_compressor.py`): Summarizes middle turns while protecting head (system prompt) and tail (recent messages). Default threshold: 66% of context window. `_decay_tool_outputs_before_tail_cut()` pre-truncates older tool outputs by age BEFORE tail boundary calculation (#13164 Option B). After 2 ineffective compressions (<10% savings), compression is skipped (anti-thrashing). Escape hatch: 2x threshold triggers forced compression.
