...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "FTS5-backed retrieval over the SQLite message store; every shape "
        "returns actual messages from the DB (no LLM calls).\n\n"
        "SOURCE-FIRST LIMIT: this searches Hermes conversation history ONLY — "
        "not evidence about current external sources. If the user gave a direct "
        "source (URL, phone/contact, app/thread, file path, account, live "
        "system), inspect that original source first when accessible; use "
        "session_search as secondary context, never as primary proof. If the "
        "source is inaccessible, say so and why before falling back. Do not "
        "conclude 'not found' from session history alone when a direct source "
        "was provided.\n\n"
        "FOUR SHAPES:\n"
        "  1) DISCOVERY — session_search(query=\"...\", limit=3): FTS5, dedupes "
        "by session lineage, top N sessions. Each result: session_id, title, "
        "when, source, FTS5-highlighted snippet, bookend_start (first 3 "
        "user+assistant msgs), messages (±5 around the FTS5 hit with "
        "match_message_id flagged), bookend_end (last 3 msgs), "
        "messages_before/after. Bookends+window reconstruct goal → match → "
        "resolution without the whole transcript.\n"
        "  2) SCROLL — session_search(session_id=..., around_message_id=..., "
        "window=10): ±window messages centered on the anchor, no FTS5. To go "
        "forward pass messages[-1].id back; backward pass messages[0].id; "
        "boundary message appears in both windows; messages_before/after < "
        "window means you're at an end.\n"
        "  3) READ — session_search(session_id=\"...\", profile=\"work\"): full "
        "session by id (first 20 + last 10 when large). Resolves an "
        "`@session:<profile>/<id>` link: split on '/' into profile + id.\n"
        "  4) BROWSE — session_search() with no args: recent sessions "
        "chronologically (titles, previews, timestamps) for \"what was I "
        "working on\" questions.\n\n"
        "LINKING: to refer the user to a session, write its `link` value "
        "verbatim inline (e.g. `@session:default/20260722_204335_d62c16`) — "
        "do not reformat as markdown or wrap in backticks; the link renders as "
        "the session title, so use it as a noun mid-sentence, never alone on "
        "its own line or beside the title/id/date spelled out.\n\n"
        "FTS5 SYNTAX: AND is the default — multi-word queries require all "
        "terms. Use OR for broader recall (`alpha OR beta`), quoted phrases "
        "for exact match (`\"docker networking\"`), boolean (`python NOT "
        "java`), or prefix wildcards (`deploy*`).\n\n"
        "WHEN TO USE: questions about Hermes conversation history itself — "
        "\"what did we do about X\", \"where did we leave Y\", \"find the "
        "session where Z\". The session DB carries what was said when; "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "limit": {
                "type": "integer",
                "description": (
                    "Discovery only. Max sessions to return (default 3, max "
                    "10). Bump to 5-10 when the topic likely spans several "
                    "sessions."
                ),
                "default": 3,
            },
            "sort": {
                ...
                "enum": ["newest", "oldest"],
                "description": (
                    "Discovery only. Temporal bias on FTS5 ranking; omit for "
                    "relevance-only order (exploratory recall). 'newest' for "
                    "recent-work, 'oldest' for origin questions. Ignored in "
                    "scroll shape."
                ),
            },
            "session_id": {
            ...
            "role_filter": {
                "type": "string",
                "description": (
                    "Comma-separated roles to include. Discovery defaults to "
                    "'user,assistant' (tool output is usually noise). Pass "
                    "'user,assistant,tool' to include tool output, or 'tool' "
                    "to search tool output only."
                ),
            },
            "profile": {
                "type": "string",
                "description": (
                    "Read sessions from another Hermes profile's database "
                    "(read-only). Use when resolving an @session:<profile>/<id> "
                    "link: pass profile with session_id. Omit for current profile."
                ),
            },
        },
        "required": [],
