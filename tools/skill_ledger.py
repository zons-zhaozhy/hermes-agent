...
# merge: upstream's constant block kept — local added ``.locks`` to ``_NON_PACKAGE_TOPS`` and
# upstream has the same member plus the ledger-cap / blob-GC / TRANSIENT_DIRS constants, which
# the already-merged (upstream-only) call sites at lines 127/175/490 require.
