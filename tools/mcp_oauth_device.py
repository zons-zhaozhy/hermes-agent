...
async def _discover(client, provider):
    ...
    for url in build_protected_resource_metadata_discovery_urls(challenge, context.server_url):
        ...
        if prm:
            ...
            break
    # merge: upstream's block kept — same multi-auth-server loop as local, but
    # ``_device_metadata`` takes the metadata-issuer helper from the repo
    # (``metadata_issued_by_origin``) instead of the SDK's exact-string check.
...
async def _device_metadata(client, server_url, auth_server_url):
    ...
    for url in build_oauth_authorization_server_metadata_discovery_urls(auth_server_url, server_url):
        ...
        # merge: upstream's check kept — root-slash-normalized issuer comparison via
        # metadata_issued_by_origin (handles Google's trailing-slash issuer, #116233 family);
        # the local side called the SDK's exact-string validate_metadata_issuer.
