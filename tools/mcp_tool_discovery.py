...
def _log_summary(prefix: str, names, **lazy) -> None:
    ...
    if new_tool_count or failed or lazy.get("lazy_servers"):
        ...
        # merge: upstream's summary line kept — it names each failing server inline (#114746);
        # the local side emitted the raw list repr (``[('n', 'reason')] failed``).
        if failed:
    ...
    try:
        with _core._lock:
            ...
            new_server_names = [name for name, cfg in servers.items()
                                # merge: upstream's mcp_server_enabled kept — the local ``_enabled``
                                # is the pre-rename spelling and no longer exists in this module.
...
def get_mcp_status(configured: Optional[Dict[str, dict]] = None, *, include_runtime: bool = True) -> List[dict]:
    # merge: upstream's body kept — same docstring/status vocabulary as local (both added
    # ``lazy``), plus the owner-secret-scope around the config load.
