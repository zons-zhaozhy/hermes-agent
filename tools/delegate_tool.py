...
def _build_top_level_description() -> str:
    ...
    if max_depth >= 2 and orchestrator_on:
        nesting_clause = (
            f"Nested delegation IS enabled for this user: pass "
            f"role='orchestrator' on a child to let it spawn its own "
            f"workers (up to {max_depth - 1} additional level(s) deep)."
        )
    elif max_depth >= 2 and not orchestrator_on:
        nesting_clause = (
            f"Nested delegation is DISABLED on this install "
            f"(delegation.orchestrator_enabled=false). role='orchestrator' "
            f"is silently forced to 'leaf'."
        )
    else:
        nesting_clause = (
            "Nested delegation is OFF for this user: every child is a leaf "
            "and cannot delegate further. Enable nesting via config."
        )

    return (
        "Spawn one or more subagents in isolated contexts. "
        "Each gets its own conversation, terminal session, and toolset; only "
        "the final summary returns.\n\n"
        "TWO MODES (one of 'goal' or 'tasks' required):\n"
        "1. Single: 'goal' (+ optional 'context', 'role').\n"
        "2. Batch: 'tasks' array (parallel, bounded by your configured "
        f"concurrency limit). {nesting_clause}\n\n"
        "BOTH MODES RUN IN THE BACKGROUND: delegate_task returns immediately; "
        "the result re-enters the conversation when done. Do NOT wait or "
        "poll; continue other work. Response includes 'live_transcripts' "
        "— per-task log under cache/delegation/live/.\n\n"
        "WHEN TO USE: reasoning-heavy subtasks (debugging, code review, "
        "research); context-flooding work; parallel workstreams.\n\n"
        "WHEN NOT TO USE: mechanical multi-step work -> execute_code; a "
        "single tool call -> call it directly; tasks needing user "
        "interaction (subagents cannot use clarify); durable long-running "
        "work -> cronjob or terminal(background=True, "
        "notify_on_complete=True). Delegations are NOT durable: parent "
        "close (/new) or process exit discards work; /stop cancels all "
        "running subagents.\n\n"
        "- Subagents have NO memory of your conversation — pass everything "
        "via 'context'.\n"
        "- Match the user's language: say so in 'context' (e.g. "
        "\"respond in Chinese\"); otherwise summaries default to English "
        "and contaminate your reply.\n"
        "- Summaries are SELF-REPORTS, not verified facts. For side-"
        "effectful ops, require a verifiable handle (URL, ID, path, HTTP "
        "status) and verify — fetch the URL, stat the file, read back — "
        "before reporting success.\n"
        "- Leaf subagents (role='leaf', default) CANNOT call: delegate_task, "
        "clarify, memory, send_message.\n"
        "- Orchestrators (role='orchestrator') retain delegate_task to spawn "
        "their own workers but still cannot use clarify/memory/send_message; "
        "disable globally via delegation.orchestrator_enabled=false.\n"
        "- Model NOT selectable per call: children inherit the parent model "
        ...
        "properties": {
            ...
            "background": {
                ...
                "description": (
                    ...
                    "background automatically — do not set this. The result "
                    "re-enters the conversation when the work finishes. "
                    "Parameter kept only for backward compatibility."
                ),
            },
            "tier": {
                "type": "string",
