...
class MemoryStore:
    ...
    # merge: both sides kept — upstream's new ``_batch_failure`` (required by apply_batch) plus
    # the local ``action`` kwarg on ``_mutate`` (the audit-ledger verb read by ``_ledger_record``).
    def _mutate(self, target: str, mutate, *, skip_drift: bool = False, action: str = "update") -> Dict[str, Any]:
        ...
        with self._file_lock(path):
            ...
            # merge: upstream's HERMES_HOME-guarded mkdir (replaces the raw path.parent.mkdir)
            # plus the local audit-ledger pre-capture of the pre-mutation content.
            from hermes_constants import mkdir_under_hermes_home

            mkdir_under_hermes_home(path.parent)
            before = self._ledger_snapshot(target)
            self._write_file(path, result[0])
            # merge: local audit-ledger record + upstream's ``extra_fields`` passthrough
            # (replaced_entry / replaced_entries / removed_entries reach the caller).
    ...
    def _edit(self, target: str, old_text: str, new_content: Optional[str]) -> Dict[str, Any]:
        ...
        def _apply(entries, limit):
            ...
            # merge: upstream's 3-tuple (replaced_entry echo) + the local ledger action verb.
    ...
    def apply_batch(self, target: str, operations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def _apply(entries, limit):
            ...
            # merge: upstream's replaced/removed-entry echo + the local ledger action verb.
