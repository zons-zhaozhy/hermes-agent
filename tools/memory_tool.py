...
MEMORY_SCHEMA = {
    "name": "memory",
    "description": (
        "Save durable facts to persistent memory that survive across "
        "sessions. Memory is injected into every future turn — keep "
        "entries compact and high-signal.\n\n"
        "HOW: make ALL changes in ONE call via an 'operations' array "
        "(each item: {action, content?, old_text?}) — applies atomically, "
        "char limit checked on the final result, so one call can remove "
        "stale entries AND add new ones even when an add alone would "
        "overflow. Use bare action/content/old_text only for a single "
        "lone change; don't repeat the call.\n\n"
        "WHEN: save proactively on user preferences, corrections, personal "
        "details, or stable facts about their environment/conventions/"
        "workflow. Priority: user prefs & corrections > environment facts "
        "> procedures. Best memory stops the user repeating themselves.\n\n"
        "IF FULL: add is rejected with current entries shown — reissue as "
        "ONE batch that removes/shortens stale entries and adds together.\n\n"
        ...
        "task progress, completed-work logs, TODO state (use session_search "
        "for those). Reusable procedures belong in a skill, not memory."
    ),
    "parameters": {
        "type": "object",
        "properties": {
