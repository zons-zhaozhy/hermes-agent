...
class GatewayGoalsMixin:
    ...
    async def _run_post_turn_hooks(
        ...
        # Tool-call evidence for the goal judge (rejects "done" claims with no tool output
        # behind them); from the turn's own transcript when the result carries one.
        _tcs = None
        with suppress(Exception):
            from hermes_cli.goals import extract_tool_calls_summary as _extract_tcs
            _conv_msgs = agent_result.get("messages") if isinstance(agent_result, dict) else None
            _tcs = _extract_tcs(_conv_msgs)
        try:
            session_entry = await self.async_session_store.get_or_create_session(
