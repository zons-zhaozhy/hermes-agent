...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    # 合并取舍/merge-take: 上游把本地的"未知子键一律拒绝"收窄为"仅拒绝前缀写错"（#112003）——
    # DEFAULT_CONFIG 不是完整 schema，运行时读取的设置可能没有种子默认值，一律拒绝会挡住合法写入；
    # 本地 _validate_config_key 侧的 gateway.discord.<field> 改正建议保留，前缀写错仍被拒。
