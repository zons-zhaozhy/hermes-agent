...
def _configured_provider_matches(
    ...
    # provider.
    # 合并取舍/merge-take: 本地方案(HEAD)在候选层剔除 provider_key 已注册为顶层 ``providers:``
    # 的 custom 项；上游改为在匹配层用 rows/_duplicates_configured_row 折叠同一 provider 的两份视图
    # (#112788)。采用上游方案：候选层剔除会丢掉仅在 legacy 行声明的模型路由。
