...
def _suppress_closed_loop_errors(loop, context) -> None:
    """Silence benign errors raised while the event loop shuts down.

    Ported with the single-query finalize (it lived in cli.py). Known teardown noise sources:
    - RuntimeError "Event loop is closed" — asyncio tasks cancelled after loop shutdown
      (MCP servers, httpx transport __del__, ...).
    - KeyError "N is not registered" — broken stdin fd on macOS with uv-managed Python (#6393).
...
def _install_closed_loop_error_suppressor() -> None:
    """Install the closed-loop suppressor for the one-shot path (cli.py did this inline).

    Interactive mode installs its own handler in ``HermesCLI.run()``; without this the
    single-query path lets httpx/httpcore transport finalizers fire during interpreter
    teardown, hit the already-closed loop and print RuntimeError to stderr. Best-effort:
    no running loop on this path is not an error.
    """
    import asyncio
    try:
        asyncio.get_running_loop().set_exception_handler(_suppress_closed_loop_errors)
    except RuntimeError:
        return  # No running loop on this path.
    except Exception as exc:
        logger.warning("closed-loop error suppressor not installed: %s", exc)


def _finalize_single_query(cli) -> None:
    """Close one-shot CLI resources before releasing the active session lease."""
    _install_closed_loop_error_suppressor()
